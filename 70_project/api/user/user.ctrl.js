...
const signup = (req, res) => {
    ...
    UserModel.findOne({ email }, (err, result) => {
        ...
        // 사용자 정보 등록
        // 암호화 종류 2가지 : 단방향, 양방향(공개 키, 비공개 키) : 구분 : 복호화 가능한가? 단방향의 경우 복호화 X, 양방향의 경우 복호화 O
        // 단방향 암호화 : 해시
        const saltRounds = 10;
        bcrypt.hash(password, saltRounds, (err, hash) => {
            if (err) return res.status(500).send("암호화 시 오류가 발생했습니다.");
            const user = new UserModel({ name: name, email: email, password: hash }) // 생략시키면 안됨. (이거): (이거) 가 같은 경우만 생략
...
// 로그인
// - 성공 : email, password가 일치하면 200(OK) 리턴
...
const checkAuth = (req, res, next) => {
    ...
    jwt.verify(token, "secretKey", (err, _id) => {
        ...
        UserModel.findOne({ _id, token }, (err, user) => {
            ...
            res.locals.user = { name: user.name, email: user.email, role: user.role };
            next();
        });
    });
};
